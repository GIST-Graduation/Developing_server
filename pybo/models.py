...
class Graduation(models.Model):
    ...
    def __init__(self):
        ...
        # 내가 들은 강좌들을 담는 딕셔너리
        self.my_classified_courses = {category: [] for category in [
            "core_english1", "core_english2", "core_writing",
            "HUS", "PPE", "other_humanity", "software",
            "core_math1", "core_math2", "core_science", "core_experiment",
            "freshman_seminar",
    ...
    @classmethod
    def classify_my_course(cls, my_major, self, my_course_index, my_courses):
        # my_course = (code, credit, title)
        my_major = cls(my_major = my_major)
        ...
        for category in ["core_english1", "core_english2", "core_math1",
            ...
            if my_course[0] in self.classified_courses[category]:
                self.my_classified_courses_credit[category] += my_course[1]
                self.my_classified_courses[category].append(my_course)
        ...
        for category in ["core_writing", "core_math2", "core_science"]:
            ...
            if my_course[0] == "GS1401":
                self.classified_courses_credit["software"] = 0
                self.courses_text["software"] = "Optional"

            if my_course[0] in self.classified_courses[category]:

                # 기초 과목을 초과해서 들으면 자유 학점으로 처리
                if my_course[1] + self.my_classified_courses_credit[category] > self.classified_courses_credit[category]:
                    self.my_classified_courses_credit["others3"] += my_course[1]
                    self.my_classified_courses["others3"].append(my_course)
        ...
        if (my_course[0] in self.classified_courses["HUS"] and (
                my_course[0] in self.classified_courses["PPE"] or my_course[0] in self.classified_courses["other_humanity"])) or (
                my_course[0] in self.classified_courses["PPE"] and my_course[0] in self.classified_courses["other_humanity"]):
            # 이해 못함
        ...
        for category in ["HUS", "PPE"]:
            ...
            if my_course[0] in self.classified_courses[category]:
                ...
                if my_course[1] + self.my_classified_courses_credit[category] > self.classified_courses_credit[category]:
                    ...
                    if my_course[1] + self.my_classified_courses_credit["other_humanity"] > self.classified_courses_credit[
                        "other_humanity"]:
                        self.my_classified_courses_credit["others2"] = self.my_classified_courses_credit["others2"] + my_course[1]
                        self.my_classified_courses["others2"].append(my_course)

                    else:
                        self.my_classified_courses_credit["other_humanity"] += my_course[1]
                        self.my_classified_courses["other_humanity"].append(my_course)
        ...
        if my_course[0] in self.classified_courses["other_humanity"]:

            if my_course[1] + self.my_classified_courses_credit["other_humanity"] > self.classified_courses_credit[
                "other_humanity"]:
                self.my_classified_courses_credit["others2"] += my_course[1]
                self.my_classified_courses["others2"].append(my_course)
        ...
        for classified_category, category in zip(Graduation.major[int(my_major)],
                                                 ["major_core", "major_elective"]):

            if my_course[0] in Graduation.classified_courses[classified_category]:
                self.my_classified_courses_credit[category] += my_course[1]
                self.my_classified_courses[category].append(my_course)
        ...
        for classified_category in [category for sublist in Graduation.major[:int(my_major)] + Graduation.major[int(my_major) + 1:] for category in
                                    sublist]:
        ...
        for code in Graduation.classified_courses["research"]:

            if my_course[0][2:] == code:
                self.my_classified_courses_credit["research"] += my_course[1]
                self.my_classified_courses["research"].append(my_course)

                return True

        for category in ["music", "exercise", "colloquium"]:

            if my_course[0] in Graduation.classified_courses[category]:
                self.my_classified_courses_credit[category] += 1
                self.my_classified_courses[category].append(my_course)
    ...
    def sum_credits(self):
        ...
        for category in ["core_english1", "core_english2", "core_writing",
            ...
            ret += min(self.my_classified_courses_credit[category],
                       Graduation.classified_courses_credit[category])

        # 전공 필수+ 선택, 전공 최대 학점 중 최솟값을 출력 (전공보다 더들었을 경우 학점 계산 X를 위함)
        ret += min(self.my_classified_courses_credit["major_core"] + self.my_classified_courses_credit["major_elective"],
                   Graduation.classified_courses_credit["major"])
        # 위와 비슷
        ret += (self.my_classified_courses_credit["others1_optional"] + self.my_classified_courses_credit["others3"])
